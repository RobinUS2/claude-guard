...
func TestDecide_RunnerCommand_PopulatesFileContent(t *testing.T) {
	// Use python3 — go run hits tier 2 instant-allow before file context
	// extraction, so it never reaches the LLM.
	f := filepath.Join(t.TempDir(), "test.py")
	...
	if captured.FileContent == "" {
		t.Fatal("expected FileContent to be populated for python3")
	}
...
func TestDecide_RunnerLargeFile_NoFileContent(t *testing.T) {
	...
	f := filepath.Join(t.TempDir(), "big.py")
